...
namespace Photon.Voice.PUN
{
    ...
    public class PhotonVoiceNetwork : VoiceConnection
    {
        ...
        private RoomOptions voiceRoomOptions = new RoomOptions { IsVisible = false }; // TODO: improve to save bandwith?
        ...
        {
            //LogFormat("OnPunStateChanged from {0} to {1}", fromState, toState);
            switch (toState)
            {
        ...
        {
            //LogFormat("OnVoiceStateChanged from {0} to {1}", fromState, toState);
            switch (toState)
            {
                case ClientState.Joined:
                case ClientState.Disconnected:
                case ClientState.ConnectedToMasterserver:
                    clientCalledDisconnect = this.Client.DisconnectedCause == DisconnectCause.DisconnectByClientLogic;
                    FollowPun();
                    break;
            }
        }
