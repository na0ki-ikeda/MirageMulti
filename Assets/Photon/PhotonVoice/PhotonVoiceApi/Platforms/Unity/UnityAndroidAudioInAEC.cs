...
namespace Photon.Voice.Unity
{
    ...
    public class UnityAndroidAudioInAEC : Voice.IAudioPusher<short>
    {
        ...
        public void SetCallback(Action<short[]> callback, ObjectFactory<short[], int> bufferFactory)
        {
            ...
            Debug.LogFormat("UnityAndroidAudioInAEC: AndroidJavaObject created: aecAvailable: {0}, minBufSize: {1}", aecAvailable, minBufSize);
            ...
            Debug.LogFormat("UnityAndroidAudioInAEC: AndroidJavaObject started: {0}, buffer size: {1}, sampling rate: {2}, channels: {3}, record buffer size: {4}, aec: {5}", ok, voiceFrameSize, SamplingRate, Channels, minBufSize * 4, aecAvailable);
        }
