...
namespace ExitGames.Client.Photon.Voice
{
    public static class AudioUtil
    {
        ...
        public static void ForceToStereo<T>(T[] src, T[] dst, int srcChannels)
        {
            ...
            {
                dst[j] = src[i]; // always fill first dst channel with data from first source channel
                dst[j + 1] = srcChannels > 1 ? src[i + 1] : src[i]; // fill second dst channel with data from second source channel, if source has at least 2 channels, otherwise with a copy of the data from the first src channel
